...
def sac(env_fn, logger_kwargs=dict(), network_params=dict(), rl_params=dict()):
    ...
    with tf.variable_scope('main', reuse=True):
        _, _, action_probs_next, log_action_probs_next, _, _, _, _  =  build_models(x2_ph, a_ph, act_dim, network_params)
    ...
    with tf.variable_scope('target'):
        ...
        _, _, _, _, q1_logits_targ, q2_logits_targ, _, _  = build_models(x2_ph, a_ph, act_dim, network_params)
    ...
    # Setup model saving
    logger.setup_tf_saver(sess, inputs={'x_ph': x_ph, 'a_ph': a_ph, 'action_probs':action_probs,
